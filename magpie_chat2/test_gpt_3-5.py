...
def preprocess_document(doc):
    ...
    for line in lines:
        ...
        if len(key_value) == 2:
            ...
            if key == 'Id':
                current_section = f"Document ID: {value}"
                processed_lines.append(current_section)
            elif key == 'Zeit_start':
                processed_lines.append(f"Zeitraum:\n  Start: {value}")
            elif key == 'Zeit_ende':
                processed_lines.append(f"  Ende: {value}")
            elif key == 'Reichweite':
                reichweite_parts = value.split(',')
                if len(reichweite_parts) >= 2:
                    sector = reichweite_parts[0].strip()
                    country = reichweite_parts[1].strip()
                    details = ', '.join(reichweite_parts[2:]).strip()
                    processed_lines.append(f"Reichweite:\n  Wirtschaftssektor: {sector}\n  Bereich: {details}")
                else:
                    processed_lines.append(f"Reichweite: {value}")
            elif key == 'Wert':
                processed_lines.append(f"Wert:\n  Betrag: {value}")
            elif key == 'Wert_einheit':
                processed_lines[-1] += f"\n  Einheit: {value}"
            elif key == 'Tag':
                processed_lines.append(f"Tags: {value}")
            else:
...
def perform_qa_with_few_shot(query, vectordb, examples, k=5):
    ...
    # Perform QA using OpenAI GPT-3.5
...
result1 = perform_qa_with_few_shot(query1, vectordb, examples, k=5)
print(result1)
...
result2 = perform_qa_with_few_shot(query2, vectordb, examples, k=5)
print(result2)

query3 = "Wie hoch waren die Externen FuE im Ausland im Jahr 2019 von Deutschlan in Tsd. Euro"
result3 = perform_qa_with_few_shot(query3, vectordb, examples, k=5)
print(result3)
